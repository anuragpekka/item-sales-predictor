...
from store.entity.artifact_entity import ModelTrainerArtifact, ModelEvaluationArtifact, ModelPusherArtifact
...
class Pipeline(Thread):
    #Class variables. These values will be same for all the objects of Pipeline.
    #"experiment.running_status" will be used to check if Pipeline is running 
    ...
    def run_pipeline(self):
        try:
            ...
            #Updatnig Pipeline.experiment after Pipeline completion
    ...
    #Appends experiment inforamtion to a csv file
    def save_experiment(self): 
        try:
            if Pipeline.experiment.experiment_id is not None:
                experiment = Pipeline.experiment
                experiment_dict = experiment._asdict()
                #Putting values in a List like, 
                # {'experiment_id': ['exp1'], 'initialization_timestamp': ['20122022042333'], 'artifact_time_stamp': ['201220220053323']}
    ...
    #Returns last few experiment information
    @classmethod
    def get_experiments_status(cls, limit: int = 5) -> pd.DataFrame:
        try:
            if os.path.exists(Pipeline.experiment_file_path):
                df = pd.read_csv(Pipeline.experiment_file_path)
                limit = -1 * int(limit)     #Setting limit to last 5 lines
